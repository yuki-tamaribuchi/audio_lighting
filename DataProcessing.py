...
class DataProcessing():
        ...
        if self.__concurrent_mode:
            ...
            def future_execute(self):
                with futures.ThreadPoolExecutor(max_workers=2) as executer:
                    results.append(executer.submit(librosa.effects.hpss,self.__loaded_data[:,0]))
                    results.append(executer.submit(librosa.effects.hpss,self.__loaded_data[:,1]))
            future_execute(self)
    ...
    def __chromacens_execute(self,n_bins=48,hop_length=4096,fmin=130.813):
        print('Chroma Cens Start')
        C_left=librosa.cqt(self.__hpss_harmonics_left,n_bins=n_bins,fmin=fmin,hop_length=hop_length)
        C_right=librosa.cqt(self.__hpss_harmonics_right,n_bins=n_bins,fmin=fmin,hop_length=hop_length)
        self.__cens_left=librosa.feature.chroma_cens(C=C_left,fmin=fmin)
        self.__cens_right=librosa.feature.chroma_cens(C=C_right,fmin=fmin)
